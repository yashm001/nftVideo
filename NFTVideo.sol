...
interface ITokenManager {
    function castVote(address creator,address owner, address voter, uint8 response) external;
    function sendToken(address tokenOwner,address from, address to, uint256 amount) external;
...
contract NFTVideo is ERC721URIStorage, Ownable {
    ...
    // struct Vote{
    //     address voter;
    //     uint8 response;   //0-> not voted 1-> upvoted 2-> downvoted
    // }
    ...
    mapping(uint256 => mapping(string => mapping(address => int256))) private _voteResponse;     //0-> not voted 1-> upvoted 2-> downvoted
    ...
    function getAllQuestion(uint256 tokenId) external view returns( uint256 , string[] memory){
        require(_exists(tokenId) , "tokenId does not exists");
    ...
    function getRank(uint256 tokenId, string memory questionHash) external view returns(int256) {
        require(_exists(tokenId) , "tokenId does not exists");
    ...
    function transferVotingPower(string memory hash, address to) external returns(bool) {
        require(_votingPower[_msgSender()][hash] !=-1, "caller has already voted or transfered power");
        _votingPower[to][hash] = _votingPower[to][hash] + _votingPower[_msgSender()][hash];
    ...
    function getVotingPower(string memory hash, address voter) external view returns(int256) {
    ...
    function upvote(uint256 tokenId, string memory questionHash) external {
        ...
        uint256 questionId = getQuestionID(tokenId, questionHash);
        require(_voteResponse[tokenId][questionHash][_msgSender()] ==0,"Voter has already voted");
        
        int256 power = _votingPower[_msgSender()][questionHash];
        require(power!=-1, "Voter has already transfered voting power for that hash");
        ...
        // question.rank = question.rank.add(1);
         _ranks[tokenId].rank[questionId] =  _ranks[tokenId].rank[questionId] + (power+1);
    ...
    function clearVote(uint256 tokenId, string memory questionHash)  external {
        ...
        // Question memory question = _ranks[tokenId][questionId];
        if(response>0){
            _ranks[tokenId].rank[questionId] =  _ranks[tokenId].rank[questionId] - response;
            _votingPower[_msgSender()][questionHash] = _votingPower[_msgSender()][questionHash] + response;
        } else{
            _ranks[tokenId].rank[questionId] =  _ranks[tokenId].rank[questionId] + response;
            _votingPower[_msgSender()][questionHash] = _votingPower[_msgSender()][questionHash] - response ;
    ...
    function getVoterResponse(uint256 tokenId, string memory questionHash, address voter) external view returns(int256) {
        require(_exists(tokenId) , "tokenId does not exists");
    ...
    function getQuestionID(uint256 tokenId, string memory questionHash) view internal returns(uint256) {
        uint256 questionCount = _tokenDetails[tokenId].numberOfQuestions;

        for(uint256 i=0 ; i<questionCount; i++) {
            if(keccak256(bytes(_ranks[tokenId].questionHash[i])) == keccak256(bytes(questionHash))){
                return i;
            }
        }
    ...
    function _exists(uint256 tokenId, string memory questionHash) internal view returns(bool) {
        uint256 questionCount = _tokenDetails[tokenId].numberOfQuestions;
    ...
    function startAuction(uint256 tokenId, uint256 startBid, uint256 AuctionTime) external {
        require(_msgSender() == ownerOf(tokenId), "Video: Caller is not owner of NFT");
    ...
    function placeBid(uint256 tokenId, uint256 amount) external biddingEnabled(tokenId) {
